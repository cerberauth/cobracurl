...
// RegisterClientFlags registers only the flags relevant to HTTP client configuration:
// TLS/security, proxy, timeouts, redirects, and network settings. Use this when
// you need to configure the client (e.g. for a scanner) without exposing
// request-level flags (method, body, headers, etc.).
...
// RegisterFlags registers all curl-compatible flags, including both client-level
// and request-level flags. For client-only flag registration, use RegisterClientFlags.
